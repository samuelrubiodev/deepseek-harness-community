...
describe('web command-line provider', () => {
  ...
  it('accepts the all-interfaces host with a warning and provides the startup values', async () => {
  ...
  it('merges DSH_TRUSTED_HOSTS from environment into trustedHosts', async () => {
    process.env.DSH_TRUSTED_HOSTS = 'env.internal, 192.168.1.10'
    try {
      const { values } = await bootProvider(['--trusted-host', 'cli.internal'])
      expect(values?.trustedHosts).toEqual(['cli.internal', 'env.internal', '192.168.1.10'])
    } finally {

...
it('forwards all other commands unchanged and retains package diagnostics', async () => {
  ...
  const combined = stderr.mock.calls.map(call => call[0]).join('')
  expect(combined).toContain('pnpm was not found')
  expect(combined).toContain('git-hosted plugins build on install')
})

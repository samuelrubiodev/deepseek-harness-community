...
不想自行构建？fork 在 `ghcr.io/samuelrubiodev/deepseek-harness-community` 发布多架构（amd64/arm64）镜像至 GitHub Container Registry (GHCR)，包含标签 `:stable`（最新发布版本）、`:latest`（`master` 默认分支的最新构建）以及 `:dsh-v<version>`（固定的具体发布版本，如 `dsh-v0.1.7-alpha.2-community.1`）；[deploy/nas/](deploy/nas/README.md) 中的模板直接匿名拉取，无需登录、检出或构建——面向 NAS（Synology、Unraid、TrueNAS）与服务器。
